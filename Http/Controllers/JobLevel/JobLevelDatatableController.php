...
class JobLevelDatatableController extends Controller
{
    public function datatable()
    {
        $religion = JobLevel::orderBy('name', 'ASC')->get();
        ...
        return DataTables::of($religion)
            ...
            ->editColumn('action', function ($religion) {
                $edit  = '<a href="' . route('job-level.edit', $religion->id) . '"
                                class="btn btn-xs btn-info">
                                <i class="fa fa-edit" aria-hidden="true"></i>
                            </a>';

                $delete = '<a data-href="' . route('job-level.destroy', $religion->id) . '" data-toggle="modal" data-target="#confirm-delete-modal"
